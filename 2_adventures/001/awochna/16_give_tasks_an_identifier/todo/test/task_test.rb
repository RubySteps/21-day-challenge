...
class TaskTest < MiniTest::Test
  ...
  def test_simple_add
    ...
    assert_match(/#{Date.today} Clean desk./, task.to_s)
    refute(task.done)
  ...
  def test_being_done_still_has_priority
    ...
    assert(task.done)
    assert_equal('A', task.priority)
    assert_match(/x \(A\) #{Date.today} Clean desk./, task.to_s)
  end
  ...
  def test_having_a_added_date
    ...
    assert_match(/#{yesterday} Clean desk./, task.to_s)
  end
  ...
  def test_adding_with_completed_date
    ...
    assert_match(/x #{Date.today} #{yesterday} Clean desk./, task.to_s)
  end
  ...
  def test_marking_done_adds_completed_date
    ...
    assert_match(/x #{Date.today} #{Date.today} Clean desk./, task.to_s)
  end
