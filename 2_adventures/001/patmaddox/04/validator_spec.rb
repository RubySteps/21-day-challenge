  ...
  describe '#validate_pull(pull)' do
    let(:pull) do
      {
        ...
        filenames: [
          ...
          '2_adventures/004/github-user/01/README.md'
        ]
      }
    end
    ...
    context 'a valid pull' do
      ...
      it 'allows README.txt as an alternative to README.md' do
        pull[:filenames] = ['2_adventures/004/github-user/01/README.txt']

        expect(response[:success]).to eq(true), response[:message]
      end

      it 'allows README as an alternative to README.md' do
        pull[:filenames] = ['2_adventures/004/github-user/01/README']

        expect(response[:success]).to eq(true), response[:message]
      end

      it 'ignores case for README files' do
        pull[:filenames] = ['2_adventures/004/github-user/01/readme.md']

        expect(response[:success]).to eq(true), response[:message]
      end

      it 'ignores case for the username' do
        pull[:filenames] << '2_adventures/004/GitHub-User/01/file.rb'

        expect(response[:success]).to eq(true), response[:message]
      end

      it 'allows a - extension for a day directory' do
        pull[:filenames] = ['2_adventures/004/github-user/01-ext/readme.md']

        expect(response[:success]).to eq(true), response[:message]
      end

      it 'allows a _ extension for a day directory' do
        pull[:filenames] = ['2_adventures/004/github-user/01_ext/readme.md']
    ...
    context 'an invalid pull' do
      ...
      it 'checks for a README.md in the day directory' do
        pull[:filenames] = ['2_adventures/004/github-user/01/NOT_README']
